...
class MinkUNetSparseAttention(nn.Module):
    ...
    def __init__(self, *,
        ...
        assert patch_factor in (1, 2, 4, 10)
    ...
    def forward_one(self, x, is_training=True, img_size=(500,500), masks=None):
        ...
        if self.patch_factor > 1:
            ...
            B_dense, Hp, Wp, D = d_out.shape
            if B_dense < orig_batch_size:
                # sparse repr can drop all-zero samples; pad with zeros to restore full batch
                pad = torch.zeros(orig_batch_size - B_dense, Hp, Wp, D, device=d_out.device, dtype=d_out.dtype)
