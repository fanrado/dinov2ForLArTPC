...
class SSLMetaArch(nn.Module):
    def __init__(self, cfg):
        ...
        if cfg.student.pretrained_weights:
            ...
            if isinstance(chkpt, dict) and "model" in chkpt and isinstance(chkpt["model"], dict):
                state_dict = chkpt["model"]
            elif isinstance(chkpt, dict) and all(isinstance(v, torch.Tensor) for v in chkpt.values()):
                state_dict = chkpt
            else:
                logger.warning("Unexpected checkpoint format; attempting to use as state_dict directly.")
                state_dict = chkpt

            # Filter out keys with incompatible shapes (e.g., loading ViT-G into ViT-L)
            filtered_state_dict = {}
            for k, v in state_dict.items():
                mod = student_backbone
                try:
                    # Walk modules to find parameter buffer with same name
                    # nn.Module.load_state_dict will handle names; here just check shape if exists
                    target_tensor = None
                    if k in mod.state_dict():
                        target_tensor = mod.state_dict()[k]
                    if target_tensor is not None and tuple(target_tensor.shape) == tuple(v.shape):
                        filtered_state_dict[k] = v
                    else:
                        logger.debug(f"Skipping key due to shape mismatch: {k} ckpt={tuple(v.shape)} model={None if target_tensor is None else tuple(target_tensor.shape)}")
                except Exception:
                    # If any error, skip this key
                    logger.debug(f"Skipping key (error while checking): {k}")

            if len(filtered_state_dict) == 0:
                logger.warning("No matching parameters found in checkpoint after shape filtering; proceeding with original state_dict.")
                filtered_state_dict = state_dict

            missing, unexpected = student_backbone.load_state_dict(filtered_state_dict, strict=False)
            if missing:
                logger.info(f"Pretrain load: missing keys count={len(missing)} (OK for heads/pos_embed if shapes differ)")
            if unexpected:
                logger.info(f"Pretrain load: unexpected keys count={len(unexpected)} (ignored)")
    ...
    def forward_backward(self, images, teacher_temp):
        ...
        def get_teacher_output():
            ...
            ibot_teacher_patch_tokens = teacher_backbone_output_dict["x_norm_patchtokens"]  # teacher patch tokens before head
            ...
            if do_ibot and not self.ibot_separate_head:
                ...
                tokens_after_head = self.teacher.dino_head(buffer_tensor_teacher)  # teacher head outputs (CLS + masked patches)
                teacher_cls_tokens_after_head = tokens_after_head[:n_cls_tokens]  # teacher CLS after head (logits)
                masked_teacher_patch_tokens_after_head = tokens_after_head[
                    n_cls_tokens : n_cls_tokens + n_masked_patches  # teacher masked patch logits after head
                ]
                ...
                teacher_cls_tokens_after_head = self.teacher.dino_head(teacher_cls_tokens)  # teacher CLS logits after head
                masked_teacher_ibot_softmaxed_centered = None

            if self.cfg.train.centering == "centering":
                teacher_dino_softmaxed_centered_list = self.dino_loss.softmax_center_teacher(
                    teacher_cls_tokens_after_head, teacher_temp=teacher_temp
                ).view(n_global_crops_teacher, -1, *teacher_cls_tokens_after_head.shape[1:])  # teacher targets for DINO
                ...
                if do_ibot:
                    ...
                    )  # teacher targets for iBOT (masked patches)
                ...
                teacher_dino_softmaxed_centered_list = self.dino_loss.sinkhorn_knopp_teacher(
                    teacher_cls_tokens_after_head, teacher_temp=teacher_temp
                ).view(n_global_crops_teacher, -1, *teacher_cls_tokens_after_head.shape[1:])  # SK teacher targets for DINO
                ...
                if do_ibot:
                    ...
                    )  # SK teacher targets for iBOT (masked patches)
        ...
        )  # student backbone outputs for global/local crops

        inputs_for_student_head_list = []

        # 1a: local crops cls tokens
        student_local_cls_tokens = student_local_backbone_output_dict["x_norm_clstoken"]  # student local CLS before head
        ...
        if do_ibot:
            ...
            ibot_student_patch_tokens = student_global_backbone_output_dict["x_norm_patchtokens"]  # student patch tokens before head
            ...
            if not self.ibot_separate_head:
                ...
                student_global_masked_patch_tokens_after_head = self.student.ibot_head(buffer_tensor_patch_tokens)[
                    :n_masked_patches
                ]  # student masked patch logits after iBOT head
        # print('---------Student backbone forward done ------')
        # 2: run
        _attn_bias, cat_inputs = fmha.BlockDiagonalMask.from_tensor_list(inputs_for_student_head_list)
        outputs_list = _attn_bias.split(self.student.dino_head(cat_inputs))  # student head outputs (batched)

        # 3a: local crops cls tokens
        student_local_cls_tokens_after_head = outputs_list.pop(0).squeeze(0)  # student local CLS logits after head

        # 3b: global crops cls tokens
        student_global_cls_tokens_after_head = outputs_list.pop(0).squeeze(0)  # student global CLS logits after head

        # 3c: global crops patch tokens
        if do_ibot and not self.ibot_separate_head:
            student_global_masked_patch_tokens_after_head = outputs_list.pop(0).squeeze(0)[:n_masked_patches]  # student masked patch logits after head

        if n_local_crops > 0:
            dino_local_crops_loss = self.dino_loss(  # DINO loss on local crops (student vs teacher targets)
            ...
            # accumulate loss
            loss_accumulator += self.dino_loss_weight * dino_local_crops_loss  # accumulate weighted DINO local loss
        ...
        if do_dino:
            ...
            dino_global_crops_loss = (
                self.dino_loss(  # DINO loss on global crops (student vs teacher targets)
            ...
            # accumulate loss
            loss_accumulator += self.dino_loss_weight * dino_global_crops_loss  # accumulate weighted DINO global loss
            ...
            if self.do_koleo:
                ...
                )  # KOLEO regularization between different images' CLS tokens
                loss_accumulator += koleo_loss  # accumulate KOLEO
        ...
        if do_ibot:
            # compute loss
            ibot_patch_loss = (
                self.ibot_patch_loss.forward_masked(  # iBOT masked patch prediction loss (student vs teacher masked targets)
            ...
            # accumulate loss
            loss_accumulator += self.ibot_loss_weight * ibot_patch_loss  # accumulate weighted iBOT loss

        self.backprop_loss(loss_accumulator)  # backprop combined loss
