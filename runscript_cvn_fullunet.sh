...
torchrun --standalone --nproc_per_node=2 \
  ...
  \
  student.arch=unet_autoencoder teacher.arch=unet_autoencoder \
  student.in_chans=3 student.unet_base=64 student.embed_dim=384 student.out_stride=4 \
  model.patch_size=4
